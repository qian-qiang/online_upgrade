...
#define CAN_INT_TX_TIMEOUT  (2000) 
...
{   
    ...
    while (msgs)
    {
        ...
        rt_completion_wait(&(tx_tosnd->completion), CAN_INT_TX_TIMEOUT);
...
rt_inline int _can_int_tx_priv(struct rt_can_device *can, const struct rt_can_msg *data, int msgs)
{
    ...
    while (msgs)
    {
        ...
        if ((tx_fifo->buffer[no].result != RT_CAN_SND_RESULT_OK))
        {
            ...
            rt_completion_wait(&(tx_fifo->buffer[no].completion), CAN_INT_TX_TIMEOUT);
            continue;
        }
